...
G_sysest_cont_tf = G_sysest_cont(__);
controller_tf = tf(controller);
L_Controlled = controller_tf*G_sysest_cont_tf;
CL_Controlled = L_Controlled/(1+L_Controlled);
  
[mag_sysest,pha_sysest] = bode(G_sysest_cont_tf, omega);
[mag_CL,pha_CL] = bode(CL_Controlled, omega);

mag_sysest_tf = mag_sysest(1,:,:);           %extraction of the mag of base, first dynamic
mag_sysest_tf = squeeze (mag_sysest_tf);
mag_sysest_tf = 20*log10(mag_sysest_tf);

mag_CL_tf = mag_CL(1,:,:);           %extraction of the mag of base, first dynamic
mag_CL_tf = squeeze (mag_CL_tf);
mag_CL_tf = 20*log10(mag_CL_tf);

mag_CL_data = 20*log10(abs(y1_max ./u_max));

figure;
semilogx(omega,mag_sysest_tf,"r", omega,mag_CL_tf,"g", omega,mag_CL_data,"xk");grid on;
