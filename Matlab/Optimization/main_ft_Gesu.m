...
theta_cvx(6,1) == -(TaoCons*Km*Kg+Beq)/Jeq; %agrgue esta

theta_cvx(7,1) == Ks/Jeq; 
theta_cvx(7,1) * Jeq == -theta_cvx(15,1)/((Jeq+JL)/(JL*Jeq)); % 

theta_cvx(8,1) == BL/Jeq; %agregue esta siguiendo la logica de la anterior
...
theta_cvx(17,1) == 0;
theta_cvx(19,1) == 0;
theta_cvx(18,1) == TaoCons/Jeq;%agregue esta
...
% Ks/Jeq
nm_est           =   -(A_cvx(2,2)*Jeq + Beq)/(TaoCons/(ng*nm)); %hice cambios aqui
Beq_est          =   A_cvx(4,2)*Jeq+ TaoCons*Km*Kg/(ng*nm)*nm_est; %hice cambios aqui
...
plot(Time_vec(2:end),Y(1:4:end))
grid on
hold on

plot(Time_vec(2:end),PHI(1:4:end,:)*theta_cvx)
