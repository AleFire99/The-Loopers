...
\label{cha:model_identification}
    ...
    \section{Mathematical Model Creation}
        ...
        \subsection{Flexible Joint and The Gearbox Equations}
            ...
            \subsubsection{Non-Linear Model for The Spring}
                ...
                \[
                    F_x = F \cdot cos(\varphi_0)\]
                \[
                    x_0 = norm (P_1 - P_0)\]

                On the right instead you are no longer in an equilibrium position, so:
                \[
                    x_k = norm(P_1-P_0) \rightarrow \Delta x_k = x_k - x_0\]
                \[
                    P_1^{NEXT} = [L cos(\alpha), L(1-sin(\alpha))] \rightarrow F_x = F \cdot cos(\varphi + \alpha)\]

                Plotting the value of the $F_x$ in function of the angle $\alpha$ compared with a linear increase we obtain:
    ...
    \section{Identification Technique}
        ...
        \subsection{Model Identification using CVX}
            ...
            \image[0.8]{./images/inertia_computation.png}
