...
\label{cha:model_identification}
    ...
    The system is composed by a coupling of a DC electric motor, schematically the system is:

    \image{./images/scheme.png}

    The physical equations of The DC Motor follow the static motor dynamic (neglecting the $L_a \cdot p I_a$ component), in fact that the resonance frequency is pretty higher \wrt the maximum frequency that the system can acquire.
    ...
    \[
        \tau = \frac{\eta_m\eta_gK_tK_g(V-K_gK_m\dot\theta)}{R_m} \]

    To model the inertia and the friction of the gearbox instead we consider their equivalent effect using the coefficient $J_{eq}$ and $B_{eq}$. The turret
    \[
        \]
