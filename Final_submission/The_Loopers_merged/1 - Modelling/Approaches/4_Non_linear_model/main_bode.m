%this scrip:
%1. load system (requires .mat)
%2. load real sine tests on model (uses load_mat.m)(requires .mat) 
%3. calculate fourier transforms
%4. apply real input to linear and nonlinear model (uses springcomp_double.m)
%5. bodeplot linear, nonlinear and real system
%6. save scripts
...
elseif logscale_flag==1
    ...
    G_1_nl = 20*log10(abs(y1_max_nl ./u_max_nl));
    magbase = mag(1,:,:);           %extraction of the mag of base, first dynamic
    magbase = squeeze (magbase);
    magbase = 20*log10(magbase);

    G_2 = 20*log10(abs(y2_max ./u_max)*(2*pi/4096));
    G_2_nl = 20*log10(abs(y2_max_nl ./u_max_nl));
    magtip = mag(2,:,:);           %extraction of the mag of tip, second dynamic
    magtip = squeeze (magtip);
...
figure(1);
semilogx(omega,magbase,"r",omega,G_1,"k" ,omega_nl,G_1_nl,"b");
title("Plot of the Base Frequency Responce");
xlabel("Frequency [ras/s]");
ylabel("Gain");
legend('Estimated Model','Real Data', 'Nonlinear Model');
grid on
figure(2);
semilogx(omega,magtip,"r",omega,G_2,"k" ,omega_nl,G_2_nl,"b");
...
saveas(figure(1), "Base Frequency Responce.png")
saveas(figure(2), "Tip Frequency Responce.png")
